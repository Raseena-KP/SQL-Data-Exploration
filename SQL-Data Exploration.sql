...
select continent,avg(population) as avg_population,avg(new_cases) as avg_new,avg(new_deaths) as avg_death
from CovidDeaths$
group by continent
order by continent desc
...
--lloking at lotal cases vs population
...
--we want to calculate evrything for entire world
--GLOBAL NUMBERS
...
--add a running total for each continent
...
--select statemnet need to run with cte command
